...
function themDongDongBo() {
  ...
  var response = ui.prompt(
    ...
    '┌─────────────────────────────────┐\n' +
    '│ Dòng ' + CONFIG.DONG_HEADER + ': Tiêu đề (Stt, Nội dung...) │\n' +
    '│ Dòng ' + CONFIG.DONG_DU_LIEU_BAT_DAU + ': PHẦN I THÔNG TIN CHUNG    │\n' +
    '│ Dòng cuối hiện tại: ' + currentLastRow + '           │\n' +
    '└─────────────────────────────────┘\n\n' +
  ...
  if (insertAfterRow < CONFIG.DONG_DU_LIEU_BAT_DAU - 1) {
    ui.alert(
      '❌ Không thể chèn trước dòng ' + CONFIG.DONG_DU_LIEU_BAT_DAU + '!\n' +
      'Chỉ chèn từ dòng ' + (CONFIG.DONG_DU_LIEU_BAT_DAU - 1) + ' trở đi.'
    );
    return;
  }
  if (insertAfterRow > currentLastRow) {
    ui.alert(
      '❌ Dòng ' + insertAfterRow + ' vượt quá số dòng hiện có (' + currentLastRow + ')!'
    );
    return;
  }
  ...
  var banNames  = banSheets.map(function(s) { return s.getName(); });
  ...
  try {
    ...
    banSheets.forEach(function(sheet) {
      ...
      sheet.getRange(insertAfterRow, 1, 1, CONFIG.COT_GHI_CHU)
           .copyTo(
      ...
      sheet.getRange(newRow, CONFIG.COT_LUY_KE)
    ...
    sheetTong.getRange(insertAfterRow, 1, 1, CONFIG.COT_GHI_CHU)
             .copyTo(
    ...
    for (var col = CONFIG.COT_DAU_KY; col <= CONFIG.COT_T12; col++) {
      sheetTong.getRange(newRow, col)
               .setFormula(xayDungCongThucTongHop(newRow, col, banNames));
    }

    // Đặt công thức Luỹ kế cho BIỂU TỔNG
    sheetTong.getRange(newRow, CONFIG.COT_LUY_KE)
             .setFormula(xayDungCongThucLuyKe(newRow));

    // ── THÔNG BÁO KẾT QUẢ ──
    ui.alert(
      '✅ THÊM DÒNG THÀNH CÔNG!\n\n' +
      '📍 Dòng mới tại vị trí:   ' + newRow          + '\n' +
      '🏘️ Sheet đã đồng bộ:       ' + banSheets.length + ' bản + BIỂU TỔNG\n\n' +
      '⚡ VIỆC CẦN LÀM TIẾP THEO:\n' +
      '→ Vào BIỂU TỔNG, dòng ' + newRow + '\n' +
...
function xoaDongDongBo() {
  ...
  var response = ui.prompt(
    ...
    '(Phạm vi hợp lệ: ' + CONFIG.DONG_DU_LIEU_BAT_DAU + ' - ' + currentLastRow + ')',
    ui.ButtonSet.OK_CANCEL
  );
  ...
  if (deleteRow < CONFIG.DONG_DU_LIEU_BAT_DAU) {
    ui.alert(
      ...
      'Chỉ xóa từ dòng ' + CONFIG.DONG_DU_LIEU_BAT_DAU + ' trở đi.'
    );
    return;
  }
  if (deleteRow > currentLastRow) {
    ui.alert(
      '❌ Dòng ' + deleteRow + ' vượt quá số dòng hiện có (' + currentLastRow + ')!'
    );
    return;
  }

  // Lấy nội dung dòng để xác nhận
  var noiDungDong = sheetTong.getRange(deleteRow, CONFIG.COT_NOI_DUNG).getValue();
  var dvtDong     = sheetTong.getRange(deleteRow, CONFIG.COT_DVT).getValue();
  var moTaDong    = (noiDungDong || '(trống)') + (dvtDong ? ' [' + dvtDong + ']' : '');

  // ── XÁC NHẬN LẦN 2 ──
  var confirm = ui.alert(
    '⚠️ XÁC NHẬN XÓA DÒNG ' + deleteRow,
    'Dòng sẽ bị xóa:\n' +
    '────────────────────────\n' +
    '"' + moTaDong + '"\n' +
    '────────────────────────\n\n' +
    '⚠️ Dòng này sẽ bị xóa khỏi BIỂU TỔNG\n' +
    '    VÀ tất cả ' + laySheetBan(ss).length + ' sheet bản!\n\n' +
  ...
  try {
    ...
    ui.alert(
      '✅ ĐÃ XÓA THÀNH CÔNG!\n\n' +
      '🗑️ Đã xóa dòng ' + deleteRow + ': "' + moTaDong + '"\n' +
      '🏘️ Đồng bộ từ ' + (banSheets.length + 1) + ' sheets'
    );
